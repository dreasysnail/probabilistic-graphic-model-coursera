...
function [P, logZ] = CliqueTreeCalibrate(P, isMax)
    ...
    if (nargout > 1)
        if (isMax)
            error ('Cannot compute logZ when performing max-sum calibration');
        end
    ...
    if (doLogZ)
        %%% YOUR CODE HERE:
        allProd = FactorProduct(unnormalizedMessages(lastCliqueOne, lastCliqueTwo), ...
                                    unnormalizedMessages(lastCliqueTwo, lastCliqueOne));
        logZ = log(sum(allProd.val));
    else
        logZ = 0;
    end
